...
def postprocess_resume_additional_report(report_path):
    """
    Post-process the Excel report for additional resume extraction fields:
    - Set header row background to green
    - Set extracted columns text color (green/red) based on match with expected
    - Auto-resize columns
    ...
    header = [cell.value for cell in ws[1]]
    extract_expected_pairs = [
        ('Expected SecondaryEmail', 'Extracted SecondaryEmail'),
        ('Expected SecondaryPhone', 'Extracted SecondaryPhone'),
        ('Expected Gender', 'Extracted Gender'),
        ('Expected MaritalStatus', 'Extracted MaritalStatus'),
    ...
    for exp_col, ext_col in extract_expected_pairs:
        if exp_col in header and ext_col in header:
            ...
            for row in range(2, ws.max_row + 1):
                ...
                # For complex fields like EducationalDetails and ExperienceDetails,
    ...
    for col in ws.columns:
        ...
        # Set maximum width limits for different column types
        if 'ExperienceDetails' in str(column_name) or 'EducationalDetails' in str(column_name):
            # Limit experience and educational details columns to 60 characters
            adjusted_width = min(max_length + 2, 60)
        elif 'SecondaryEmail' in str(column_name):
            # Limit email columns to 35 characters
            adjusted_width = min(max_length + 2, 35)
        elif 'SecondaryPhone' in str(column_name):
            # Limit phone columns to 20 characters
            adjusted_width = min(max_length + 2, 20)
        elif 'Gender' in str(column_name) or 'MaritalStatus' in str(column_name):
            # Limit gender and marital status columns to 15 characters
            adjusted_width = min(max_length + 2, 15)
        elif 'DOB' in str(column_name) or 'PAN' in str(column_name) or 'Passport' in str(column_name):
            # Limit date and ID columns to 20 characters
            adjusted_width = min(max_length + 2, 20)
        else:
            # For other columns, use normal sizing with a reasonable max
    ...
    # Add overall statistics row
    summary_ws.append(
        ['Overall Wrong Cells', f"{total_wrong_cells}/{total_extracted_cells}", f"{overall_wrong_percentage:.1f}%"])
    overall_row = len(summary_data) + 3  # +3 because of header + empty row + overall row
    for col_idx in range(1, 4):
        summary_ws.cell(row=overall_row, column=col_idx).fill = overall_fill
        summary_ws.cell(row=overall_row, column=col_idx).font = bold_font
